...
int fibo_data[100];			// 피보나치 - 순환 (속도개선)

int fibo_fast(int n)
{
	for (int i = 0; i < 100; i++)	fibo_data[i] = 0;

	if (n == 0)	return 0;
	if (n == 1) return 1;

	if (fibo_data[n] != 0)	return fibo_data[n];	// 이미 저장된 것이 있으면,
	else {
		fibo_data[n] = fibo_recursion(n - 2) + fibo_recursion(n - 1);
		return fibo_data[n];
	}
}
